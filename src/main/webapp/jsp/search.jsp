...
<script type="text/javascript">
	function jump(){
		...
		if(go != ""){
	 		window.location.href="/HelloWeb/web/page?where="+go;
		}
	}
</script>
</head>
...
<body>
	<form method="post" action="/HelloWeb/web/search">
	...
	if(list != null){
		...
		out.print("<a href='/HelloWeb/web/page?where="+((currentPage-1) < 1 ? 1 : (currentPage-1)) +"'>上一页</a>");
		out.print("<a href='/HelloWeb/web/page?where="+((currentPage+1) > pages ? pages : (currentPage+1)) +"'>下一页</a>");
		...
		for(ListEntry entry : list){
			...
			out.print("<br/><a href='/HelloWeb/web/goInfo?url="+entry.getUrl()+"' title='"+entry.getTitle()+"' target='_blank'><img src='"+entry.getImages()+"' alt='"+entry.getTitle()+"' height='400px'></a>");
			out.print("</li>");
		}
		out.print("</ul><br/>");
	}
